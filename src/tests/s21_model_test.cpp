...
TEST(constructor_first, constructor_first_true) {
  ...
  EXPECT_EQ(model.express_answer(), "15.4+16.7*4.532mod5=16.0844");
  EXPECT_EQ(model.get_result_exp(), "16.0844");
}

TEST(constructor_second, constructor_second_true) {
  s21::Model model("sin(cos(X25", 0.5);
  EXPECT_EQ(model.express_answer(), "sin(cos(0.5*25))=0.84027935");
  EXPECT_EQ(model.get_result_exp(), "0.84027935");
}

TEST(constructor_copy, constructor_copy_true) {
  s21::Model model("cos(sin(X", 1.23);
  s21::Model copy(model);
  EXPECT_EQ(model.get_input(), copy.get_input());
  EXPECT_EQ(model.get_x(), copy.get_x());
}

TEST(constructor_moving, constructor_movin_true) {
  s21::Model model("cos(sin(X", 1.23);
  s21::Model copy = std::move(model);
  EXPECT_EQ(model.get_input(), "");
  EXPECT_EQ(copy.get_input(), "cos(sin(X");
}
...
TEST(validity, validity_false) {
  ...
  EXPECT_EQ(model.express_answer(), "error");
}
...
TEST(validity_3, validity_false) {
  ...
  EXPECT_EQ(model.express_answer(), "sin(-2-3)=0.95892427");
}

TEST(validity_4, validity_false) {
  s21::Model model("atan(-", 1.23);
  EXPECT_EQ(model.express_answer(), "");
}
...
TEST(parse, parse_true) {
  s21::Model model(".+3");
  EXPECT_EQ(model.express_answer(), ".0+3=3");
}

TEST(correct_answer, correct_answer_true) {
  s21::Model model("atan(0.5");
  EXPECT_NEAR(model.solution_without_checks(), 0.463647609, 1e-7);
}

TEST(correct_answer_2, correct_answer_true) {
  s21::Model model("sqrt(16");
  EXPECT_NEAR(model.solution_without_checks(), 4, 1e-7);
}
...
TEST(correct_answer_4, correct_answer_true) {
  ...
  EXPECT_EQ(model.express_answer(), "--2+3=5");
}

TEST(correct_answer_5, correct_answer_true) {
  s21::Model model("-2+-2");
  EXPECT_EQ(model.express_answer(), "-2+-2=-4");
}

TEST(correct_answer_6, correct_answer_true) {
  s21::Model model("-sqrt(-X)", -2);
  EXPECT_EQ(model.express_answer(), "-sqrt(--2)=-1.41421356");
}
...
TEST(correct_answer_8, correct_answer_true) {
  s21::Model model("sin(.");
  EXPECT_EQ(model.express_answer(), "sin(.0)=0");
}
...
TEST(solution_without_checks, solution_without_checks_true) {
  s21::Model model("cos(sin(X", 1.23);
  EXPECT_NEAR(model.solution_without_checks(), 0.58777634833, 1e-7);
}

TEST(valid_graph, valid_graph_false) {
  s21::Model model("cos(sin-))");
  EXPECT_FALSE(model.is_valid_for_graph());
}
